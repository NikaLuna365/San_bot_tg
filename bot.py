...
# После теста: выбор действий и режим общения с Gemini
...
def build_gemini_prompt_for_test(test_answers: dict) -> str:
    ...
    standard = (
        ...
        "Клиент прошёл ежедневный тест. "
        "Проанализируйте его ответы, используя научные термины и принципы когнитивной психологии, "
        "и сформируйте краткий анализ состояния клиента."
    )
    prompt = standard + "\n\n"
...
def build_gemini_prompt_for_followup_chat(user_message: str, test_answers: dict) -> str:
    """
    Формирует промпт для последующих вопросов.
    Инструкция для ИИ:
      - Используй только данные сегодняшнего теста.
      - Ответь только на вопрос клиента, не повторяя общий обзор теста.
      - Ответ должен быть предельно кратким (не более 120 слов).
      - Не используй никаких символов форматирования (например, звездочки, подчеркивания, тильды, обратные апострофы, вертикальные черты, HTML-теги и т.п.). Выдай обычный текст.
    """
    standard = (
        "Вы профессиональный психолог с 10-летним стажем. Клиент уже получил анализ своего состояния на основе сегодняшнего теста. "
        "Теперь ответьте на следующий вопрос, используя только данные теста. "
        "Не повторяйте общий обзор, а дайте конкретные рекомендации, отвечая исключительно на заданный вопрос. "
        "Ответ должен быть максимально кратким и уложиться в 120 слов. "
        "Никаких символов форматирования (звездочек, подчеркиваний, тильд, обратных апострофов, вертикальных черт, HTML-тегов) использовать не нужно."
    )
    prompt = standard + "\n\nДанные теста:\n"
    all_questions = FIXED_QUESTIONS + OPEN_QUESTIONS
    for i, question in enumerate(all_questions, start=1):
...
def build_gemini_prompt_for_retro(averages: dict, test_count: int) -> str:
    ...
    standard = (
        "Вы профессиональный психолог с 10-летним стажем. "
        "Клиент прислал недельный тест. "
        "Проанализируйте изменения в его состоянии за неделю, используя научные термины и принципы когнитивной психологии, "
        "и дайте развернутый анализ эмоционального состояния."
    )
    prompt = standard + "\n\n"
...
async def gemini_chat_handler(update: Update, context: CallbackContext) -> int:
    ...
    Формируется новый промпт, основываясь только на вопросе клиента и результатах сегодняшнего теста.
    Ответ должен быть конкретным, кратким (не более 120 слов) и без форматирования.
    """
